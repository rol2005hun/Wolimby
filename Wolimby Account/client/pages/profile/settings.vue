        ...
        <div class="settings-box">
            ...
            <div class="right-section">
                <div class="section account">
                    ...
                    <form @submit.prevent="saveUser(currentUser._id, 'profile', '')" v-if="currentUser.profile.username"
                        class="change form">
                        <div class="change-row">
                            <input type="text" v-model="currentUser.profile.username" class="change-input" id="ch-us"
                                minlength="3" maxlength="20" required>
                            <label for="ch-us" class="change-label">Felhasználónév</label>
                        </div>
                        <div class="change-row">
                            <input type="text" v-model="currentUser.profile.name" class="change-input" id="ch-na"
                                minlength="4" maxlength="20" required>
                            <label for="ch-na" class="change-label">Teljes név</label>
                        </div>
                        <div class="change-row">
                            <input type="email" v-model="currentUser.profile.email" class="change-input" id="ch-em"
                                minlength="6" maxlength="30" required>
                            <label for="ch-em" class="change-label">Email cím</label>
                        </div>
                        <div class="change-row">
                            <input type="text" v-model="currentUser.profile.biography" class="change-input" id="ch-bg"
                ...
                <div class="section appearance">
                    ...
                    <div class="bg-image">
                        ...
                        <div class="upload-img">
                            <input type="file" id="upload-img" @change="changeImage($event, 'bgimage.value')"
                                accept="image/*">
                            <i class="fa fa-trash"
                                @click="saveUser(currentUser._id, 'appearance', 'deletebgimage.value')"></i>
                            <label for="upload-img">
                                <i class="fa fa-file-image"></i>
                            </label>
                            <img :src="previewbg ? previewbg : bgimage" alt="bg-image">
                        </div>
                        <button type="submit" @click="saveUser(currentUser._id, 'appearance', 'setbgimage')"
                            class="submit-btn">
                            Mentés
                        </button>
                    </div>
                    <div class="set-theme">
                        ...
                        <a @click="saveUser(currentUser._id, 'appearance', { setting: 'theme', theme: 'theme1' })">Téma
                            1.</a>
                        <a @click="saveUser(currentUser._id, 'appearance', { setting: 'theme', theme: 'theme2' })">Téma
                            2.</a>
                        <a @click="saveUser(currentUser._id, 'appearance', { setting: 'theme', theme: 'theme3' })">Téma
                            3.</a>
                        <a @click="saveUser(currentUser._id, 'appearance', { setting: 'theme', theme: 'theme4' })">Téma
                            4.</a>
                    </div>
                    <hr>
                    <div class="profile-iframe">
                        <h3>Profil Iframe</h3>
                        <h4>A Profile Iframe a <router-link to="/profile">Profilod</router-link>-hoz hasonló kártya
                            szerűség,
                ...
                <div class="section notifications">
                    <h2><i class="fa fa-refresh" @click="userStore.getCurrentUser"></i> Értesítések kezelése <i
                            class="fa fa-trash" @click="removeNotifications(currentUser._id)"></i></h2>
                    <div class="section-notifications">
                        <div class="notification" v-for="notification in currentUser.profile.notificationList"
                            :key="notification._id" :title="(formatDateToText(notification.createdAt))">
                ...
                <div class="section security">
                    ...
                    <div class="privacy-settings" v-if="currentUser.privacy">
                        ...
                        <label class="switch name">
                            <input type="checkbox" v-model="currentUser.privacy.showName"
                                @click="saveUser(currentUser._id,
                        ...
                        <label class="switch email">
                            <input type="checkbox" v-model="currentUser.privacy.showEmail"
                                @click="saveUser(currentUser._id,
    ...
    switch (option1) {
        ...
        case 'appearance':
            ...
            if (option2 == 'setbgimage') {
                if (bglink.value) {
                    document.body.style.backgroundImage = 'url(' + bglink.value + ')';
                ...
                patchedUserSettings = {
                    backgroundImage: bglink.value ? bglink.value : 'none',
                }
            }
...
function showPage(pageName: number) {
    ...
    const targetSection = section[pageName];
...
@use '@/assets/scss/profile/settings.scss';
</style>